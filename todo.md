...
- [ ] **`DatePicker` doesn't use the `Input` component internally** — `todoList.md` explicitly notes it should, but it just reuses `pf-field` CSS classes directly. If `Input` changes, DatePicker won't track. [`DatePicker.tsx`](packages/react/src/components/DatePicker/DatePicker.tsx)
- [ ] **`Icon` returns `null` silently for unknown names** — no dev-mode warning. A `console.warn` gated on `import.meta.env.DEV` would surface typos faster. [`Icon.tsx:197`](packages/react/src/components/Icon/Icon.tsx)
